...
import { getInstitutionId, getSocialFeedUserId } from '@/hooks/useAuthIdentity'
...
export const useClassFeed = (): UseClassFeedReturn => {
  ...
  const fetchSemesters = useCallback(async () => {
    ...
    try {
      ...
      console.error('[useClassFeed] semester error:', err)
  ...
  const fetchFeed = useCallback(
    async (opts: { reset?: boolean; semId?: number } = {}) => {
      ...
      try {
        ...
        console.error('[useClassFeed] fetch error:', err)
