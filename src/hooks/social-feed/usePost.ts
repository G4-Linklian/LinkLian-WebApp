...
export const usePosts = (sectionId: number): UsePostsReturn => {
  ...
  const fetchPosts = useCallback(
    async (opts: { reset?: boolean; type?: PostType | 'all' } = {}) => {
      ...
      const targetType = opts.type !== undefined ? opts.type : filterType;
      const apiType: PostType | undefined = targetType === 'all' ? undefined : targetType;
      ...
      try {
        ...
        try {
          ...
          if (fallback?.success) {
            ...
            setPosts((prev) => isFirstLoad ? fallbackPosts : [...prev, ...fallbackPosts]);
