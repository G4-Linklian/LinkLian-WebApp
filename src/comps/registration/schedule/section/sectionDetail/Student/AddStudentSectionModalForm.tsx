...
import { IconSelector, IconMail, IconPhone, IconId, IconLibrary, IconChalkboardTeacher } from '@tabler/icons-react';
...
interface AddStudentSectionModalFormProps {
...
export default function AddStudentSectionModalForm({
  onSubmit,
  close,
  token,
  roleID
}: AddStudentSectionModalFormProps) {
  ...
  const handleUserSelect = (value: string | null) => {
    ...
    if (selectedOption && selectedOption.originalData) {
      ...
      form.setValues({
        ...
        level_name: u.level_name || undefined,
        program_name: u.program_name || undefined,
      });
    }
  };
    ...
    <form onSubmit={form.onSubmit(handleSubmit)} className="gap-2 flex flex-col">
      ...
      <Grid gutter="sm">
        ...
        <Grid.Col span={6}>
          <TextInput
            label="ห้อง"
            leftSection={<IconChalkboardTeacher size={16} />}
