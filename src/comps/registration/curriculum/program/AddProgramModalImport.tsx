...
import { validateImportProgramData, saveImportProgramData } from "@/utils/api/import";
import TableProgramImport from "./TableProgramImport";
import { getProgramImportConfig } from "@/config/csvHeader";
...
export default function AddProgramModalImport() {
    ...
    useEffect(() => {
        ...
        const instTypeVal = token.institution.inst_type;
        if (token && instTd && instTypeVal) {
            setInstId(instTd);
            setInstType(instTypeVal);
        }
    }, []);

    // Get import config based on institution type
    const importConfig = useMemo(() => {
        return getProgramImportConfig(instType || "school");
    }, [instType]);

    // Get unit text based on inst_type and route
    const unitText = useMemo(() => {
        if (!instType) return "";
        
        const isUniversity = instType === "uni" || instType === "university";
        
        if (!root_id) {
            return isUniversity ? "คณะ" : "แผนการเรียน";
        } else if (twig_id) {
            return isUniversity ? "สาขา" : "ห้องเรียน";
        } else if (root_id) {
            return isUniversity ? "ภาควิชา" : "ระดับชั้น";
        }
        return importConfig.label;
    }, [instType, root_id, twig_id, importConfig.label]);
    ...
    const handleValidate = async () => {
        ...
        try {
            const data = await validateImportProgramData(
                selectedFile,
                instId!,
                instType!
            );
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const data = await saveImportProgramData(
                validationToken,
                selectedFile, 
                instId!, 
                instType!);
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
                        ...
                        <Paper withBorder p="lg" radius="md" bg="var(--mantine-color-gray-0)" mb="xl">
                            ...
                            <List size="sm" spacing="xs" c="gray.7">
                                ...
                                <List.Item>4. <b>ระบบจะเพิ่ม{unitText}ใหม่ที่ยังไม่มีในระบบ</b></List.Item>
                            </List>
                        </Paper>
                ...
                {activeStep === 3 && (
                    <div style={{
                        ...
                        <Text c="dimmed" size="sm" mb={40} style={{ maxWidth: 300 }}>
                            ระบบได้ทำการนำเข้าข้อมูล{unitText}ทั้งหมดเรียบร้อยแล้ว
