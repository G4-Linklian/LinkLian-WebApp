...
import { getInstitutionReport, updateInstitutionReport } from '@/utils/api/report';
...
export default function ReportTable({ filterParams, onResolved }: ReportTableProps) {
	...
	const handleResolve = async (report: ReportRow) => {
		if (!report.inst_report_id || !instId) return;

		setResolvingReportId(report.inst_report_id);
		try {
			const response = await updateInstitutionReport({
				inst_report_id: report.inst_report_id,
				inst_id: instId,
				mark_resolved: true,
			});
			...
			<Table.Td ta="center">
				<Group justify="center" gap="xs">
					...
					<Button
						...
						loading={resolvingReportId === item.inst_report_id}
						disabled={resolvingReportId !== null && resolvingReportId !== item.inst_report_id}
					...
					<Table.Tbody>
						{rows.length > 0 ? (
							...
							<Table.Tr>
								<Table.Td colSpan={6}>
									<Center py="md">
										<Text c="dimmed" size="sm">ยังไม่มีรายการแจ้งปัญหาที่ยังไม่แก้ไข</Text>
									</Center>
								</Table.Td>
							</Table.Tr>
						)}
