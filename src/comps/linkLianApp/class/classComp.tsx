// ─────────────────────────────────────────────
// comps/linkLianApp/class/classComp.tsx
// Component หลักสำหรับหน้า class feed
// id convention: classes-{element}
// ─────────────────────────────────────────────

import React, { useCallback, useState, useRef } from 'react';
import { Alert, Box, Button, Loader, ScrollArea, Text } from '@mantine/core';
...
import { useClassFeed } from '@/comps/linkLianApp/hook/social-feed/useClassfeed';
        ...
        <Box className="h-[calc(100vh-64px)] w-full overflow-hidden bg-[#FAFAFA] text-black">
            <div id="classes-page" className="relative h-full w-full bg-[#fafafa] px-4 pb-6 pt-4 text-black md:px-6">
                <div id="classes-content" className="mx-auto flex h-full w-full max-w-[1500px] flex-col">
                    ...
                    {/* Composer + FilterSemester row */}
                    <div id="classes-header" className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-3">
                        <div className="flex-1">
                            <ComposerBar
                                classList={classList}
                                onOpenCreate={() => setShowCreatePopup(true)}
                            />
                        </div>
